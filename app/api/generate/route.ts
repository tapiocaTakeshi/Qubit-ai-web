...
export async function POST(req: Request) {
  ...
  let history: unknown;
  try {
    ({ prompt, mode = "chat", model = "default", history = [] } = await req.json());
  } catch {
  ...
  try {
    ...
    const result = await generate(prompt.trim(), req.signal, { mode, model, history } as GenerateOptions);
